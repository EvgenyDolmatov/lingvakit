...
@section('header-tools')
    <ul class="breadcrumb">
        ...
        <li class="breadcrumb-item active">{{ __("cms-pages.edit-presentation") }}</li>
    </ul>
@endsection
@section('content')
    <form class="form-horizontal" method="POST" action="{{ route('presentations.update', [$course, $stage, $lesson, $presentation]) }}">
        @csrf @method('PUT')
                        ...
                        <div class="form-group row d-flex align-items-center mb-5">
                            ...
                            <div class="col-lg-9">
                                <input type="text" name="title" class="form-control"
                                       placeholder="{{ __("cms-pages.title") }}" value="{{old('title', $presentation->title)}}">
                                @error('title')
                        ...
                        <div class="form-group row d-flex align-items-center mb-5">
                            ...
                            <div class="col-lg-9">
                                <textarea id="description" name="description" class="form-control" rows="3"
                                          placeholder="{{ __("cms-pages.description") }}">{{old('description', $presentation->description)}}</textarea>
                                @error('description')
                                <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="col-lg-9">
                                <div class="form-group preview">
                                    <div class="current-item">
                                        <img src="{{ $presentation->getImage() }}" width="240" alt>
                                    </div>
                                </div>
                                ...
                                <div class="slides-wrap">
                                    ...
                                    @if($presentation->slides->count())
                                        @foreach($presentation->slides as $slide)
                                            <div id="{{ "slide-item-" . $slide->id }}" class="slide-item" draggable="true">
                                                <img src="{{$slide->getImage()}}" style="width:100%" alt="">
                                                <input type="hidden" name="slide_images[]" value="{{ $slide->image }}">
                                                <i class="la la-close la-2x" data-method="PUT"></i>
                                            </div>
                                        @endforeach
                                    @endif
